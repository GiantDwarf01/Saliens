...
$Output = SendGET( 'ITerritoryControlMinigameService/GetPlanets', 'active_only=1&language=english' );
$fp = fopen('results.json', 'w');
fwrite($fp, json_encode($Output));
fclose($fp);

$Output = SendGET( 'ITerritoryControlMinigameService/GetPlanets', 'language=english' );
$GameCount = 0;
$AvailableGames = [];

$GameID = file('preferredGames.txt');
$Games = [];
foreach ($GameID as $ID)
{
	Msg ('Found game ID: ' . $ID );
	$Games[ trim($ID) ] = true;
}
...
do
{
	$CurrentPlanet = GetFirstAvailablePlanet( $SkippedPlanets, $KnownPlanets, $Games );
}
...
function GetFirstAvailablePlanet( $SkippedPlanets, &$KnownPlanets, $Games )
{
	$Planets = SendGET( 'ITerritoryControlMinigameService/GetPlanets', 'active_only=1&language=english' );
	...
	foreach( $Planets as &$Planet )
	{
		...
		$Planet[ 'preferred_games' ] = 0;
		$Planet[ 'hard_zones' ] = 0;
		$Planet[ 'medium_zones' ] = 0;
		$Planet[ 'easy_zones' ] = 0;
		
		foreach ( $Planet['giveaway_apps'] as $App )
		{
			if ( isset( $Games [ $App ] ) )
				$Planet[ 'preferred_games' ]++;
		}
		...
		Msg(
			'>> Planet {green}%3d{normal} - Hard: {yellow}%2d{normal} - Medium: {yellow}%2d{normal} - Easy: {yellow}%2d{normal} - Games: {yellow}%1d{normal} - Captured: {yellow}%5s%%{normal} - Players: {yellow}%8s {green}(%s)',
			PHP_EOL,
			[
				$Planet[ 'id' ],
				$Planet[ 'hard_zones' ],
	...
	$Priority = [ 'preferred_games', 'hard_zones', 'medium_zones', 'easy_zones' ];
	...
	foreach( $Planets as &$Planet )
	{
		...
		if( !$Planet[ 'state' ][ 'captured' ]  )
		{
			...
			global $GameCount;
			$GameCount = $Planet[ 'preferred_games' ];
			return $Planet[ 'id' ];
		}
	}
